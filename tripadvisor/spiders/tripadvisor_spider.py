...
class TripadvisorSpider(scrapy.Spider):
    ...
    def parse(self, response):
        """
        From given url the main page response is scraped saving info about restaurants in the SQLite via pipelines.
        The offset is increased each request time to access next 30 restaurants' data. While scraping a restaurant's
        short info from main page, its link used to request restaurants' page which scraped in the parse_restaurants().
        """
        offset = response.request.url.split("-")[2]
        oa = 0
        if "oa" in offset:
            oa = int(offset[2:])

        if oa < self.page_offset:
            pass
        else:
            ...
            self.page_offset += self.offset_step
            self.count += 1

            if oa == 0:
                url_parts = response.request.url.split("-Az")
                self.next_page = url_parts[0] + "-oa" + str(self.page_offset) + "-Azerbaijan.html"
            else:
                url_parts = response.request.url.split("-oa")
                self.next_page = url_parts[0] + "-oa" + str(self.page_offset) + "-Azerbaijan.html"
            yield scrapy.Request(self.next_page, self.parse)
